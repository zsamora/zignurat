...
reg AS (
    ...
    SELECT b.id, 45, 112, next_index_id.id, o.id,
        ...
        'Juan', 'Soto', 'Carmen', 'Rojas', 'Manuel Herrera López', 'Isidora Contreras Muñoz',
        now(), now()
...
SELECT next_index_id.id, reg.org_baptism, reg.book_id, reg.id, 'Soto', 'Rojas', 'María', 'Fernanda', 45, now(), now()
...
reg AS (
    ...
    SELECT b.id, 78, 145, next_index_id.id, o.id,
        ...
        'Ricardo', 'Fuentes', 'Patricia', 'Vergara', 'José Muñoz Castro', 'Teresa Silva Vega',
        now(), now()
    FROM books b, organizations o, next_index_id
    WHERE b.book_nr = 12 AND o.name = 'Santo Espíritu Sagrado'
    RETURNING id, book_id, org_baptism
...
INSERT INTO index_baptisms (id, org_id, book_id, reg_id, user_surname_f, user_surname_m,
    user_name_first, user_name_second, page_number, created_at, updated_at)
SELECT next_index_id.id, reg.org_baptism, reg.book_id, reg.id, 'Fuentes', 'Vergara', 'Cristóbal', '', 78, now(), now()
FROM reg, next_index_id;
...
reg AS (
    ...
    SELECT b.id, 8, 201, next_index_id.id, o.id,
        ...
        'Álvaro', 'Espinoza', 'Daniela', 'Contreras', 'Francisco Reyes Ortiz', 'Valentina López Cerda',
        now(), now()
    FROM books b, organizations o, next_index_id
...
INSERT INTO index_baptisms (id, org_id, book_id, reg_id, user_surname_f, user_surname_m,
    user_name_first, user_name_second, page_number, created_at, updated_at)
SELECT next_index_id.id, reg.org_baptism, reg.book_id, reg.id, 'Espinoza', 'Contreras', 'Martín', 'Ignacio', 8, now(), now()
FROM reg, next_index_id;
...
reg AS (
    ...
    SELECT b.id, 12, 33, next_index_id.id, o.id,
        ...
        'Sebastián', 'Muñoz', 'Camila', 'Torres', 'Andrés Bravo Soto', 'Francisca Vera Lagos',
        now(), now()
    FROM books b, organizations o, next_index_id
...
INSERT INTO index_baptisms (id, org_id, book_id, reg_id, user_surname_f, user_surname_m,
    user_name_first, user_name_second, page_number, created_at, updated_at)
SELECT next_index_id.id, reg.org_baptism, reg.book_id, reg.id, 'Muñoz', 'Torres', 'Valentina', 'Isidora', 12, now(), now()
FROM reg, next_index_id;
...
reg AS (
    ...
    SELECT b.id, 27, 41, next_index_id.id, o.id,
        ...
        'Rodrigo', 'Salinas', 'Javiera', 'Cortés', 'Felipe Núñez Ramos', 'Constanza Peña Ortiz',
        now(), now()
    FROM books b, organizations o, next_index_id
    WHERE b.book_nr = 5 AND o.name = 'Capilla San José'
    RETURNING id, book_id, org_baptism
...
INSERT INTO index_baptisms (id, org_id, book_id, reg_id, user_surname_f, user_surname_m,
    user_name_first, user_name_second, page_number, created_at, updated_at)
SELECT next_index_id.id, reg.org_baptism, reg.book_id, reg.id, 'Salinas', 'Cortés', 'Tomás', '', 27, now(), now()
FROM reg, next_index_id;
